...
AllDatavisu <- function(object2, redmet = "PCA", cmet = "svm", ax = c(1,2),
                        ...
                        vect = FALSE, same_size = FALSE, sz = 1,
                        expor.png = FALSE, expor.pdf = FALSE, yourseed = 500){
  dname <- deparse(substitute(object2))
  ...
  } #t-SNE only work with unique features --> filter first
  else {
  ...
  if (!unknow){ #set the size of the points on the plot according their clustering score or not
    ...
    if (!same_size){
      ...
      p2$score <- as.factor(rep(sz, nrow(p2)))
    }
  }
  ...
  grap2 <- ggplot2::ggplot(data = p2, aes(x=Dim1, y=Dim2, fill=markers,
                                 size = score)) +
    ggplot2::geom_point(alpha = 0.7, shape = 21) + ggplot2::guides(alpha = "none", size = "none")

  if (is.null(MSnbase::fData(object2)[[Condition]])){  #if no condition in the data (no dynamic)
    MSnbase::fData(object2)[[Condition]] <- rep(1,nrow(fData(object2))) #create one in order to have always the same data organization for after
    Main <- Title #the plot title
    ...
    Main <- paste(Title, c("Control", "Treated"))   # creation of the plot title according to the number of condition
    if (k > 2){
      for (i in 1:(k/2)){
  ...
  if (!highpr & !vect  & !unknow){ #creating the plot according to the number of condition, then highlight the condition
    ...
    for (i in 1:k){
      message(i)
      graph1a <- grap2 + ggplot2::labs(title = Main[i],
                              subtitle = paste("data :", dname, redmet),
                              fill = "Organelles",
                              x = axname[1], y = axname[2],
                              caption = capt) +
        ggplot2::theme_bw() +
  ...
  if (highpr & !vect){ #same thing as before but with highlighting specifics proteins
    Gr <- list()
    for (i in 1:k){
      message(i)
  ...
  if (vect){ #see the moving proteins with vector
    ...
    p2_f <- p2 %>% #filter the proteins we selected
      dplyr::group_by(unprot) %>%
      dplyr::filter(!is.na(match(unprot, proteins))) %>%
      dplyr::ungroup()
    ...
    graph_v <- grap2 + ggplot2::labs(title = Main[i],
                                    subtitle = paste("data :", dname, redmet),
                                    fill = "Organelles",
                                    x = paste("Axe", ax[1]), y = paste("Axe", ax[2]),
                                    caption = capt) +
      ggplot2::theme_bw()  +
    ...
    message("3. End of plot building")

    if (expor.png){ #if you want to export the plot created in png of pdf format
      ggplot2::ggsave(paste0(dname, "_", redmet, "_vect.png"), plot = graph_v, width = 2000/300, height = 1000/300)
    }
    if (expor.pdf){
      ggplot2::ggsave(paste0(dname, "_", redmet, "_vect.pdf"), plot = graph_v, width = 2000/300, height = 1000/300)
    }
    ...
    if (expor.png){ #export the figure in png or pdf format
      ggplot2::ggsave(paste0(dname, "_", redmet, "_map.png"), plot = final_graph, width = 2000/72, height = 1000/72)
    }
    if (expor.pdf){
      ggplot2::ggsave(paste0(dname, "_", redmet, "_map.pdf"), plot = final_graph, width = 2000/72, height = 1000/72)
    }
