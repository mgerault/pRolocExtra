...
#' @param highcond A logical argument if you want tu highlight specific condition (replicates / control and treated)
...
#' library(pRolocdata)
#' data(tan2009r1)
#' AllDatavisuInt(datavisupca(tan2009r1, method ="knn", sh.gr = FALSE), cmet = "knn", Interact = TRUE)
AllDatavisuInt <- function(object2, redmet = "PCA", cmet = "svm", ax = c(1,2), Interact = FALSE,
  ...
  if (unknow == FALSE){ #set the size of the points on the plot according their clustering score or not
  ...
  if (Interact == FALSE) {
    ...
    if (vect == TRUE){ #see the moving proteins with vector
  ...
  #Interactive graph
      ...
      #creation of the plot : highlight the specifics proteins we chosen + the vectors showing their shifting
        graph1a <-ggplot2:: ggplot(p2_f) +
          ggplot2::geom_point(aes(x=Dim1, y=Dim2, fill = markers,
                         size = markers, key = prot), data = p2, colour = alpha("lightblue", 0.7), size = 0.9) +
          ggplot2::geom_point(aes(x=Dim1, y=Dim2, fill = markers,
                         size = markers, key = prot), shape= 21) + guides(alpha = FALSE, size = FALSE) +
    ...
    if (unknow) {
      ...
      graph1a <- ggpubr::ggpar(graph1a, main = Title, subtitle = paste("data :", sub, redmet),
                               ...
                               ggtheme = ggplot2::theme_minimal(), xlab = axname[1], ylab = axname[2],
                               caption = capt) +
        ggplot2::scale_fill_manual(values = c(PaletteWithoutGrey(p2$markers, 1))) +  #keep same color
        ggplot2::guides(fill = ggplot2::guide_legend(override.aes = list(size = 5))) #keep the legend
    }
      ...
      graph1a <- plotly::ggplotly(graph1a, tooltip = c("fill", "key"), dynamicTicks = TRUE, source = Source) %>%
        plotly::layout(hovermode = "closest", clickmode = "event+select", dragmode = "select",
               title = list(text = paste0(Title,
                                          ...
                                          '</sup>')), annotations = ArrowAnnot(p2_f1))
