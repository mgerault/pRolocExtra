...
#' @param method The clustering method, available : svm, knn, perTurbo, nnet (for neural network), rf (for random forest) and naiveBayes
...
datavisupca <- function(object, mfcol = "markers", method = "svm", ax = c(1,2), sh.gr = TRUE, tm = 5, cv = 5){
  ...
  if (sh.gr){
    ...
    myalpha <- matlab::linspace(0,0,length(MSnbase::fData(object2)[[mfcol]]))    #creation of a low alpha for the markers that are unknown (better visualization on the plot)
    ...
    rep.fgr <- factoextra::fviz_pca_ind(rep.pca, geom.ind = "point",                           #build the first PCA graph
      ...
      ggplot2::guides(fill = ggplot2::guide_legend(override.aes = list(size = 6)), size = FALSE)     #change the size component from the fill legend and not showing the size legend

    graph1 <- ggpubr::ggpar(rep.fgr, main = "PCA", subtitle = paste("data :", deparse(substitute(object))),          #use of the ggpar function in order to have better graph aspect
                    legend.title = "Organelles", legend.position = "top", ggtheme = ggplot2::theme_minimal()) +
      ggplot2::scale_fill_manual(values = c(PaletteWithoutGrey(MSnbase::fData(object2)[[mfcol]]), "unknown" = "darkgrey")) +            #change the color manually with PaletteWithoutGrey function and set the color of protein which are assign to unknown in grey
      ggplot2::scale_size_manual(values = c(matlab::linspace(5,5,length(unique(MSnbase::fData(object2)[[mfcol]])) -1), "unknown" = 2)) #change the size manually : the protein which are assign to unknown are smaller than the others
  }
  ...
  if (sh.gr){
    ...
    ptszerep <- exp(MSnbase::fData(object2)[[paste0(method, ".scores")]]) - 1        #get back this score and modify it in order to change size of each point on the plot according to their clustering score
    rep.loss <- round(mean(MSnbase::fData(object2)[[paste0(method, ".scores")]]),3)  #get the mean of the clustering score to have it on the plot
    ...
    rep.clus <- factoextra::fviz_pca_ind(rep.pca, geom.ind = "point",
                             pointshape = 21,
                             pointsize = ptszerep,
                             fill.ind = MSnbase::fData(object2)[[method]],
                             alpha.ind = 0.6, mean.point = FALSE, axes = ax) +
      ggplot2::guides(fill = ggplot2::guide_legend(override.aes = list(size = 5)), size = FALSE)



    graph2 <- ggpubr::ggpar(rep.clus, title = method, subtitle = paste("data :", deparse(substitute(object))),
                    legend.title = "Organelles", legend.position = "top",
                    caption = paste("Scores =", rep.loss),
                    ggtheme = ggplot2::theme_minimal(), palette = PaletteWithoutGrey(MSnbase::fData(object2)[[method]], 1))
