...
#'
#' @export
BoxProfile <- function(obj, prot = "GRB2", Condition = "cond"){

  prot_data <- as.data.frame(MSnbase::exprs(obj)[which(MSnbase::fData(obj)$gene_name == prot),])  #take the data from the protein chosen
  Loca <- unique(MSnbase::fData(obj)$markers[which(MSnbase::fData(obj)$gene_name == prot)])       #take the marker from the protein
  ...
  profile <- ggplot2::ggplot(prot_data, aes(x = fraction, y = value, colour = cond, fill = cond))+
    ...
    ggplot2::ggtitle(paste(deparse(substitute(obj)), ",", prot, "clustered in", Loca)) + #the localization in the title : the first is for the control condition, the second is for the treated (can change between the replicates if the localisation is calculated by clustering (error))
